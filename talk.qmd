...
![$d \sim 60~\mu$m, $\mathcal{S} = 0.01$](talk_files/src/videos/silibeads40m_70m/run02/output.mp4){width=80% fig-align="center"}

![$d \sim 135~\mu$m, $\mathcal{S} = 0.04$](talk_files/src/videos/silibeads120m/run02/output.mp4){width=80% fig-align="center"}

![$d \sim 250~\mu$m, $\mathcal{S} = 0.1$](talk_files/src/videos/silibeads200m_300m/run02/output.mp4){width=80% fig-align="center"}
...
## Slumping regime duration, $t_{\rm end}$

[$\bullet$ Dimensionless by settling time $t_{\rm s} = h_{0}/v_{\rm s}$]{.fragment fragment-index=4}
...
- duration $t_{\rm end}$:
  - $t_{\rm end} \propto t_{0} = L_{0}/u_{0}$ for $\mathcal{S} < 2.10^{-2}$ 
  - $t_{\rm end} \propto t_{s} = h_{0}/v_{\rm s}$ for $\mathcal{S} > 2.10^{-2}$ 
...
- Influence of $\theta$ and $\mathcal{S}$ on $E$ ?
